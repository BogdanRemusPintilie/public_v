-- Update get_ctl_portfolio_summary to include shared dataset access and keep normalization
...
BEGIN
  ...
  WITH normalized_data AS (
    SELECT
      ...
      -- Normalize PD to percentage (0-100)
      CASE 
      ...
      END AS pd_pct,
      -- Normalize LGD to fraction (0-1)
      CASE 
        WHEN ctl.lgd > 1 THEN ctl.lgd / 100 
        ELSE ctl.lgd 
      END AS lgd_frac,
      -- Normalize interest rate to percentage (0-100)
      CASE 
